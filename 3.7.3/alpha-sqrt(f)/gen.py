...
if len(csv_files) < 1:
	...
	for i in range(1):
		...
		# Column labels
		y_label = "\\alpha, {m}^{-1}"
		yerr_label = "\\sigma_{alpha}"
		x_label = "sqrt(f)"
		xerr_label = "\\sigma_{sqrt(f)}"
		...
		slope_err = 0.043  # Replace with your value
		intercept_err = 0.2  # Replace with your value


		# Plot with error bars
		plt.errorbar(
			x_data, y_data,
			yerr=1.9*y_err, xerr=x_err,
