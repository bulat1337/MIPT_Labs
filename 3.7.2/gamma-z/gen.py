...
if len(csv_files) < 1:
	...
	for i in range(1):
		...
		slope_err = 0.001  # Replace with your value
		intercept_err = 0.042  # Replace with your value
	...
	# Adjusted position to avoid overlapping with data points
	text_x = min(x_data) + 0.05 * (max(x_data) - min(x_data))  # 5% from the left edge
	text_y = max(y_data) - 0.1 * (max(y_data) - min(y_data))  # 10% below the top edge
